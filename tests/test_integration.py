...
class TestIntegration:
    ...
    def setup(self):
        ...
        if not wait_for_service(f"{API_GATEWAY_URL}/health", timeout=5):
            pytest.skip("API Gateway not available")
        if not wait_for_service(f"{OCR_SERVICE_URL}/health", timeout=5):
        ...
        try:
            ...
            if scan_response.status_code == 200:
                ...
                if len(extracted_text) > 100:
                    ...
                    quiz_request = {
                        "text": extracted_text[:500],  # Limit for testing
