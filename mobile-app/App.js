...
// Replace with your computer's IP address (e.g., 'http://192.168.1.100:8000')
...
export default function App() {
  ...
  useEffect(() => {
    // Set the API URL for the service
  ...
  const generateStudyContent = async (text) => {
    try {
      ...
      // For very long text, generate sequentially to avoid overwhelming Ollama
      // For shorter text, generate in parallel for faster results
      const isLongText = text.length > 5000;
      
      if (isLongText) {
        // Sequential generation for long text to avoid timeouts and overwhelming Ollama
        console.log('Long text detected, generating sequentially to avoid timeouts...');
        Alert.alert(
          '⏳ Processing Long Text',
          `Your text is ${text.length} characters. Processing sequentially to avoid timeouts. This will take longer but is more reliable.\n\nEstimated time: 3-5 minutes`,
          [{ text: 'OK' }]
        );
        
        // Generate quiz first with retry logic
        let quizGenerated = false;
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            console.log(`Generating quiz... (attempt ${attempt}/2)`);
            const quizResult = await StudyCoachAPI.generateQuiz(text, 'multiple_choice');
            if (quizResult && quizResult.success && quizResult.quiz) {
            ...
            console.error(`Quiz generation failed (attempt ${attempt}):`, e);
            if (attempt === 2) {
              Alert.alert('⚠️ Quiz Error', `Quiz generation failed after 2 attempts: ${e.message}\n\nThis might be due to:\n- Backend/Ollama not responding\n- Network timeout\n- Try waiting a moment and scanning again`);
            } else {
              // Wait before retry
        ...
        // Then summary with retry logic
        let summaryGenerated = false;
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            console.log(`Generating summary... (attempt ${attempt}/2)`);
            const summaryResult = await StudyCoachAPI.generateSummary(text);
            ...
            console.error(`Summary generation failed (attempt ${attempt}):`, e);
            if (attempt < 2) {
              await new Promise(resolve => setTimeout(resolve, 2000));
            }
          }
        ...
        // Finally flashcards with retry logic
        let flashcardsGenerated = false;
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            console.log(`Generating flashcards... (attempt ${attempt}/2)`);
            const flashcardsResult = await StudyCoachAPI.generateFlashcards(text);
            if (flashcardsResult && flashcardsResult.success && flashcardsResult.flashcards) {
            ...
            console.error(`Flashcards generation failed (attempt ${attempt}):`, e);
            if (attempt < 2) {
              await new Promise(resolve => setTimeout(resolve, 2000));
            }
          }
      ...
      // Parallel generation for shorter text (original behavior)
      // Generate all content in parallel, but update UI as each completes
      // Note: LLM generation can be slow (30-120 seconds per request), so be patient
      const quizPromise = StudyCoachAPI.generateQuiz(text, 'multiple_choice')
        .then(result => {
          ...
          if (result.success && result.quiz) {
            ...
            Alert.alert('⚠️ Quiz Generation', 'Quiz generation had issues. Check console for details.');
          }
          return result;
        })
        .catch(e => {
          console.error('Quiz generation failed:', e);
          Alert.alert('❌ Quiz Error', `Failed to generate quiz: ${e.message}\n\nThis might be due to:\n- LLM taking too long\n- Network timeout\n- Backend issues`);
  ...
  /**
   * Auto-detect backend by trying common local network IPs
   */
  const autoDetectBackend = async () => {
    setLoading(true);
    const commonIPs = [];
    
    // Generate common local network IPs to try
    // Try 192.168.1.x (most common home network) - limit to reasonable range
    for (let i = 1; i <= 100; i++) {
    ...
    // Helper function to test a single IP with timeout
    const testIP = (ip) => {
    ...
    for (let i = 0; i < commonIPs.length; i += batchSize) {
      ...
      if (found) {
        setLoading(false);
        const detectedUrl = `http://${found.ip}:8000`;
  ...
  const renderSettings = () => {
    const [tempUrl, setTempUrl] = React.useState(apiUrl);
