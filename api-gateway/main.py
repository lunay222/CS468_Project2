...
async def _generate_quiz_with_llm(text: str, quiz_type: str) -> Dict:
    ...
    if quiz_type in ["multiple_choice", "all"]:
        ...
        # For longer text (multiple documents), generate more questions
...
async def _generate_flashcards_with_llm(text: str) -> List[Dict]:
    ...
    # For longer text (multiple documents), generate more flashcards
    text_length = len(text)
    if text_length > 5000:
        num_flashcards = "15-20"
    elif text_length > 3000:
        num_flashcards = "12-16"
    else:
        num_flashcards = "8-12"
...
async def _call_ollama(prompt: str, system_prompt: str = None, model: str = "llama3") -> str:
    ...
    try:
        ...
        # Increased timeout to 10 minutes (600s) because LLM generation can be slow, especially for longer text
        ...
        logger.error(f"Timeout calling Ollama (took longer than 10 minutes): {str(e)}")
        raise Exception(f"LLM generation timed out after 10 minutes. The model may be too slow. Try using a smaller/faster model or shorter text.")
